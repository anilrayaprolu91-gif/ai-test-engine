...
export default function App() {
  ...
  const pushToGitHub = async (path: string, markdown: string, commitMessage: string): Promise<string> => {
    ...
    const slug = path.split('/').pop()?.replace(/\.md$/i, '') ?? 'spec';
    ...
    const { data: pr } = await octokit.pulls.create({
      ...
      head: branchName,
      base: 'main',
      body: `Auto-generated by AI Test Engine dashboard.\n\nMerging this PR will trigger the CI pipeline to generate test plans and Playwright tests.\n\n- Spec: \`${path}\``,
    });
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      ...
      const prUrl = await pushToGitHub(path, markdown, `docs(plans): add test plan for ${formValues.brdId || 'new BRD'}`);
      setPlanStatus(`Test plan ${fileName} pushed. PR opened for review: ${prUrl}`);
    } catch (err) {
  ...
  const handleCreatePlanAndTests = async () => {
    ...
    try {
      ...
      const prUrl = await pushToGitHub(path, markdown, `feat(specs): add spec for ${formValues.brdId || 'new BRD'} [generates tests]`);
      setPlanAndTestsStatus(`Spec ${fileName} pushed. PR opened: ${prUrl} — CI will generate test plan + Playwright tests on merge.`);
    } catch (err) {
  ...
  const handleGenerateTests = async () => {
    ...
    try {
      ...
      for (const brdId of selectedBrds) {
        ...
        const filePath = `specs/${ts}-${brdId.toLowerCase()}-spec.md`;
        const branchName = `feat/trigger-${brdId.toLowerCase()}-${ts}`;
        ...
        const { data: pr } = await octokit.pulls.create({
          owner: githubOwner,
          repo: githubRepo,
          title: `chore(specs): trigger test generation for ${brdId}`,
          head: branchName,
          base: 'main',
          body: `Auto-generated trigger by AI Test Engine dashboard.\n\nMerging this PR will trigger the CI pipeline to generate Playwright tests for **${brdId}**.`,
        });
        prUrls.push(pr.html_url);
      }
      setSelectedBrds([]);
      setGenerateStatus(`Opened ${prUrls.length} PR(s) for review: ${prUrls.join(' | ')}`);
    } catch (err) {
