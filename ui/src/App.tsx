...
export default function App() {
  ...
  const pushToGitHub = async (path: string, markdown: string, commitMessage: string): Promise<string> => {
    return pushFilesToGitHub([{ path, markdown }], commitMessage, `Auto-generated by AI Test Engine dashboard.\n\n- File: \`${path}\``);
  };
  ...
  const handleCreatePlanAndTests = async () => {
    ...
    setPlanAndTestsStatus(specDeliveryMode === 'github' ? 'Pushing plan + spec to GitHub in one PR...' : 'Preparing local spec file...');
    try {
      ...
      const prUrl = await pushFilesToGitHub(
        [
          { path, markdown },
          { path: planPath, markdown: planMarkdown },
        ],
        `feat(specs): add plan + spec for ${formValues.brdId || 'new BRD'} [generates tests]`,
        `Auto-generated by AI Test Engine dashboard.\n\nThis PR includes all requested files in a single change set:\n- Spec: \`${path}\`\n- Test Plan: \`${planPath}\`\n\nMerging this PR will trigger CI to generate/update Playwright tests if required.`,
      );
      setPlanAndTestsStatus(`Plan + spec pushed in one PR: ${prUrl} — CI will generate/update Playwright tests on merge.`);
    } catch (err) {
  ...
  const handleGenerateTests = async () => {
    ...
    try {
      ...
      const files: PendingGitHubFile[] = selectedBrds.map((brdId, index) => {
        ...
        const filePath = `specs/${ts + index}-${brdId.toLowerCase()}-spec.md`;
