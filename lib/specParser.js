...
function getRequirementByBRD(brdId) {
  ...
  const raw = String(brdId || '').trim().toUpperCase();
  const normalized = normalizeBrdId(brdId);

  return mapping[raw] || mapping[normalized] || null;
}
...
function getGenerationContext(brdId) {
  ...
  const normalized = normalizeBrdId(brdId);
  const mapping = loadSpecMapping();
  const resolvedBrdId = mapping[raw] ? raw : mapping[normalized] ? normalized : normalized;
  const item = mapping[resolvedBrdId] || null;
